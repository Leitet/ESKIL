...
#                            — logic + mcp + mcp-transport + mcp-klienter + laget
...
case "${1:-all}" in
  logic) exec node --test test/logic.test.js test/mcp.test.js test/mcp-transport.test.js test/mcp-klienter.test.js test/laget.test.js ;;
