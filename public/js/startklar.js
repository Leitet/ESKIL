  ...
  if (!patrols.length) {
    ...
    if (!startTider) {
      ...
      if (luckor.length) {
        ...
        varning('startlista-luckor',
          `Startlistan har ${luckor.length === 1 ? 'en lucka' : luckor.length + ' luckor'} (${tider.join(', ')}) — dra en patrull dit eller ta bort luckan i patrullistan.`,
          'patrols');
      }
